...
{
    /// <inheritdoc />
    public partial class ReseedCardArtCatalog : Migration
    {
        ...
        {
            // SeedCardArtCatalog already ran once and won't run again, but the seed CSV
            // it reads has since been corrected (COREKS/base-set reprints wrongly listed
            // as separate illustrations, FOILER insert cards, a default-art resolution
            // bug) — wipe and reload from the current (corrected) embedded resource
            // rather than requiring every environment that already migrated once to
            // drop and recreate its database.
        ...
        {
            using var resource = typeof(ReseedCardArtCatalog).Assembly.GetManifestResourceStream(ResourceName)
